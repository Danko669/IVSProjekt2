##
# @file calc.py
# @brief Westernová kalkulačka s GUI - Verze s Doxygen dokumentací a cestami k médiím.
# 
# Tento modul implementuje grafickou kalkulačku ve stylu divokého západu.
# Obsahuje pokročilé matematické funkce, zvukové efekty a vizuální easter eggy.
#
...
## @brief Absolutní cesta k adresáři, kde se nachází tento skript.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

## @brief Dynamické určení cesty ke složce media.
# Hledá složku 'media' buď o úroveň výš (vedle 'src'), nebo přímo vedle skriptu.
MEDIA_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "media"))
if not os.path.exists(MEDIA_DIR):
    MEDIA_DIR = os.path.join(BASE_DIR, "media")

try:
    ## @brief Import externích matematických funkcí z modulu mathematic.
...
if platform.system() == "Windows":
    ...
    ## @brief Pomocná funkce pro odesílání MCI příkazů na Windows.
    # @param cmd Řetězec s MCI příkazem.
    # @return Návratový kód MCI operace.
    def _mci(cmd: str) -> int:
        return _winmm.mciSendStringW(cmd, None, 0, None)
else:
    ## @brief Fallback pro systémy mimo Windows (zatím nepodporuje zvuk).
    def _mci(cmd: str) -> int:
        return 0

## @brief Spustí hudbu na pozadí ve smyčce.
...
## @brief Pozastaví přehrávání hudby na pozadí.
def pause_background_music(): _mci("pause bgmusic")

## @brief Obnoví přehrávání hudby na pozadí.
def resume_background_music(): _mci("resume bgmusic")

## @brief Úplně zastaví a uvolní hudbu na pozadí.
def stop_background_music():
    _mci("stop bgmusic")
    _mci(f"close bgmusic")

## @brief Přehraje zvuk šustění křoví.
...
## @brief Načte seznam cest k souborům se zvuky výstřelů.
# @return Seznam (list) absolutních cest k existujícím mp3 souborům.
def load_shot_paths():
...
## @brief Přehraje náhodný zvuk výstřelu na jednom z volných kanálů.
# @param shot_paths Seznam cest k dostupným zvukům.
...
# --- Konfigurace barev (Western Palette) ---
## @brief Slovník barev použitých v uživatelském rozhraní.
C = {
...
# @brief Hlavní třída aplikace zajišťující GUI a logiku kalkulačky.
class WesternCalculator:
    
    ## @brief Konstruktor třídy. Inicializuje okno, UI a spustí hudbu.
    # @param root_window Instance tk.Tk okna.
    def __init__(self, root_window: tk.Tk):
        self.root = root_window
        self.root.title("☆  DEAD MAN'S CALC  ☆")
        self.root.resizable(False, False)

        # Načtení ikonky aplikace
        try:
        ...
        self.expression = ""        ##< Aktuální matematický výraz v řetězci.
        self.result_shown = False   ##< Příznak, zda je právě zobrazen výsledek.
        self.error_state = False    ##< Příznak, zda je kalkulačka v chybovém stavu.
        self.pending_op = None      ##< Uložená operace pro binární funkce (mocnina, odmocnina).
        self.pending_val = None     ##< Uložená hodnota pro binární funkce.
        ...
        # Spuštění časovače pro létající křoví
        self.root.after(15000, self.letajici_krovi)

    ## @brief Interní obsluha zavření okna. Zastaví hudbu.
    def _on_close(self):
        stop_background_music()
        self.root.destroy()

    ## @brief Načte grafické assety pro speciální efekty (létající křoví, chyby).
    ...
    ## @brief Sestaví hlavní uživatelské rozhraní na plátně (Canvas).
    def _build_ui(self):
    ...
    ## @brief Vytvoří mřížku tlačítek podle definovaného rozložení.
    # @param serif Název vybraného fontu s patkami.
    ...
    ## @brief Vykreslí jednotlivé tlačítko na Canvas a nastaví jeho události.
    ...
    ## @brief Zobrazí vyskakovací obrázek a přehraje zvuk k danému easter eggu.
    ...
    ## @brief Periodická funkce pro spuštění animace létajícího křoví.
    ...
    ## @brief Rekurzivní funkce pro pohyb a rotaci křoví po obrazovce.
    ...
    ## @brief Aktualizuje text na hlavním displeji.
    def _show(self, v): 
        self.disp_var.set(v)
        self.canvas.itemconfig(self.disp_text_id, text=v)
        
    ## @brief Aktualizuje text pomocného displeje (výrazu).
    ...
    ## @brief Zpracuje úspěšný výsledek operace.
    # @param res Číselný výsledek.
    ...
    ## @brief Přepne kalkulačku do chybového stavu a zobrazí vizuální efekt.
    # @param msg Zpráva o chybě.
    def _error(self, msg):
        self._show("CHYBA"); self._show_expr(msg[:22]); self.error_state = True
        self._vyskakovaci_efekt("error.mp3", self.img_error, "eggerror")

    ## @brief Zpracuje stisknutí číslice nebo tečky.
    def _digit(self, d):
    ...
    ## @brief Zpracuje stisknutí operátoru (+, -, *, /).
    def _op(self, op):
    ...
    ## @brief Zpracuje unární speciální funkce (odmocnina, faktoriál, abs).
    ...
    ## @brief Připraví kalkulačku na druhý operand u binárních funkcí (mocnina, n-tá odmocnina).
    ...
    ## @brief Vypočítá aktuální výraz nebo dokončí čekající binární operaci.
    def _equals(self):
        if self.error_state: self._clear(); return
    ...
    ## @brief Přepíná stav hudby na pozadí (On/Off).
    ...
    ## @brief Získá aktuální číselnou hodnotu z displeje.
    def _current_number(self): 
        try: return float(self.disp_var.get())
        except: return 0.0
    
    ## @brief Vymaže veškerá data a stavy kalkulačky.
    ...
    ## @brief Odstraní poslední zadaný znak.
    def _backspace(self):
    ...
    ## @brief Formátuje výsledek do čitelné podoby, případně do vědecké notace.
    # @param result Číselný vstup k formátování.
    # @return String s naformátovaným výsledkem.
